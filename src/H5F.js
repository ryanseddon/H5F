...
(function (root, factory) {
    ...
    setup = function(form, settings) {
        ...
        var validator = create(settings || opts);

        if(isCollection) {
            for(var k=0,len=form.length;k<len;k++) {
                validator.validate(form[k]);
            }
        } else {
    ...
    create = function(args) {
        ...
        var d = document,
            ...
            // Methods
            validation, validity, checkField, bypassChecks, checkValidity, setCustomValidity, support, pattern, placeholder, range, required, valueMissing, listen, unlisten, preventActions, getTarget, addClass, removeClass, isHostMethod, isSiblingChecked;
